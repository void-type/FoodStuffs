...
public static class DocumentMappers
{
    ...
    public static SearchRecipesResponse ToSearchRecipesResponse(this Document doc)
    {
        ...
        (
            ...
            Ingredients: doc.GetValues(C.FIELD_INGREDIENTS_JSON)
                .Select(x =>
                {
                    var i = JsonSerializer.Deserialize<Ingredient>(x);

                    return i is not null
                        ? new SearchRecipesResponseIngredient(i.Name, i.Quantity, i.Order, i.IsCategory)
                        : null;
                })
