...
test.describe.serial('Categories CRUD', () => {
  ...
  test('verify category appears in list', async ({ page }) => {
  ...
  test('verify updated category in list', async ({ page }) => {
    await page.goto('/categories');
    await page.getByTestId('search-name').fill(TEST_MARKER);
    await page.getByTestId('search-button').click();
  ...
  test('verify category is deleted', async ({ page }) => {
    await page.goto('/categories');
    await page.getByTestId('search-name').fill(TEST_MARKER);
    await page.getByTestId('search-button').click();
    await expect(page.getByText('Found no categories.')).toBeVisible();
  });
});
